...
| `query` | string | YES | blank 금지 | 사용자 자연어 질문 또는 Supervisor가 보존한 질의 |
...
| `최근` + `N년/N개년` + 명시적 최신 키워드 없음 | `report_mode="annual"` | `report_mode:annual_recent_nyears_override` |
| `최근` + years 미매칭 | `report_mode="latest"` | `report_mode:recent_keyword` |
| `분기`, `이번 분기`, `최신` | `report_mode="latest"` | `report_mode:explicit_latest_keyword` |
| `별도`만 있음 | `fs_div="OFS"` | `fs_div:standalone` |
| `연결`만 있음 | `fs_div="CFS"` | `fs_div:consolidated` |
| `수익성`, `마진`, `ROE` | `intent="profitability"` | `intent:profitability` |
| `부채`, `안정성`, `유동` | `intent="stability"` | `intent:stability` |
| `성장`, `매출 증가` | `intent="growth"` | `intent:growth` |
| `밸류`, `PER`, `PBR`, `저평가` | `intent="valuation"` | `intent:valuation` |
| 매칭 실패 | 기본값 유지 | `defaulted_fields`에 기록 |
