...
- **`INTRADAY_MINUTE_MAX_CALLS=20`(≈600봉) 상한(정본 `config.py §14`/`config.md §12`):** 정규장(09:00–15:30 ≈391봉)은 여유 있게 커버. 이번 응답은 시간외까지 연속봉(09:46–19:45)이라 상한(600봉)에서 정지 — 정규장 범위엔 영향 없음.
