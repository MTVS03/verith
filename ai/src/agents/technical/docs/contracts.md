...
| `signal_score_adjustment` | v1은 **항상 0.0**(미조정) |
| `risk_notes` | 기존 `RiskFlag`(enums) 확장이 아니라 **`intraday_context` 내부 중립 표현 문자열 리스트**(최대 3개). 기존 `risk.items[]`와 별개 |
