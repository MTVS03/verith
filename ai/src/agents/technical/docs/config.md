...
실 LLM 어댑터(`services/openai_llm_client.py`)가 기존 `complete(prompt) -> str` 계약을 OpenAI **Responses API**(`responses.create`→`output_text`)로 구현할 때 쓰는 설정. **`.env` = secret + 환경별 선택값(API key·MODEL), config.py = 튜닝 상수**로 나눈다. 값의 단일 출처가 겹치지 않게 한다(`technical_coding_guidelines §2.2·§13.2`).
...
**config.py 튜닝 상수(코드 — secret도 환경차도 아님, `KIS_TIMEOUT_SECONDS`와 같은 급. `.env`에 넣지 않는다)**

| 상수 | 기본값 | 단위 | 의미 |
|---|---|---|---|
| `OPENAI_TIMEOUT_SECONDS` | `30.0` | 초 | 1회 호출 timeout |
...
- **예외 정책**: OpenAI SDK 예외(timeout·rate limit·auth·connection·API·BadRequest)는 모두 `LlmCallError`로 변환(메시지는 type 이름만 — API key·raw prompt·raw response 미포함). `OPENAI_API_KEY`·`OPENAI_MODEL` 누락은 `default_openai_client()` 생성 시점의 **config error(fail-fast)** 로 `LlmCallError`와 구분한다.
