...
def _make_quotes() -> pd.DataFrame:
    ...
    전일이 없어 검사 대상 밖 — 임의값 0). 거래량·외국인순매매량은 자유값.
    """
    rows = []
    prev = None
    for i in range(25):
        ...
        rows.append([close, change, rate, 500000.0 + i, 1000.0 - i])
        prev = close
...
def test_extract_price_daily_serializes_window_and_falls_back_to_none():
    ...
    claimed = signals.extract_price_daily(_quotes)
    ...
    assert claimed[-1]["frgn_qty"] == 976.0
    assert signals.extract_price_daily(None) is None
    assert signals.extract_price_daily(pd.DataFrame()) is None
